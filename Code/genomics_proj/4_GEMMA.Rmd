...
Now we have as output the results of the GWAS analysis.
...
resultGemma$chr <- as.factor(resultGemma$chr)

# compute the Bonferroni threshold
bonferroni05 <- -log10(0.05 / nrow(resultGemma))
bonferroni01 <- -log10(0.01 / nrow(resultGemma))
bonferroni001 <--log10(0.001/ nrow(resultGemma))

colors <- ifelse(as.integer(levels(resultGemma$chr)) %% 2 == 0, "firebrick", "steelblue")

custom_labels <- function(x) {
  ifelse(as.numeric(x) %% 2 == 0, as.character(x), "")
}

plot <- resultGemma %>%
  ggplot() +
  geom_jitter(aes(chr, -log(p_lrt), color = chr), size = 2, alpha = .6, show.legend = FALSE) +
  geom_hline(yintercept = bonferroni05, linetype = "dashed") +
  geom_hline(yintercept = bonferroni01, linetype = "dashed") +
  geom_hline(yintercept = bonferroni001, linetype = "dashed") +
  scale_color_manual(values = setNames(colors, levels(resultGemma$chr))) +
  scale_x_discrete(labels = custom_labels) +
  labs(x = "Chromosome") +
  theme_bw() +
  theme(aspect.ratio = 0.6)

ggsave("plot.png", width = 6, height = 4, dpi = 96)
...
is it possible to merge also the names of the canine breeds and run a geom_wrap?