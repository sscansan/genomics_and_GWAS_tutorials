...
We compute the euclidean distance between individuals (PCA).
```{r}
system("plink \\
--chr-set 29 no-xy \\
--allow-no-sex \\
...
```{r}
dist_matrix <- read.table("Data/ADAPTmap_genotypeTOP_20160222_full/out/PCA_data.mdist", header = T)
...
head(famInd)
```
And PCA on the distance matrix of the dataset.
#### With `cmdscale()`
```{r}
