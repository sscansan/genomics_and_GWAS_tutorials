...
`mydata.ped` (pedigree file) and `mydata.map` (genetic map). In this example, we will use these two from
...
So `plink --file mydata --pheno pheno.raw --assoc --maf 0.05 --out run1` becomes:
