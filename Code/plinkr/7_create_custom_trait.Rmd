...
1 SNP, 2 haplotype in a diploid organism.
...
recessive_phenotype <- function(snvs) {
  only_ts <- rowSums(snvs == "C") == ncol(snvs)
  10.0 + only_ts
}
```
...
Phenotype function of 2 or more loci (multi SNPs trait) here dependent on the two loci snv_1 and snv_2.
We'll write a custom function, called `epistatic_phenotype`,
which assumes that the trait is only expressed
for an individual that only has cytosine for the two loci:
...
E.g., when the loci are both homozygous.
