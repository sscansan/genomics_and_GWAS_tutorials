...
With the **plinkr** function `create_demo_assoc_qt_data()` we are able to generate mock simulated dataset that create **.map** table, **.ped** table and phenotype table.
 ...
 * **epistatic**: ("Any time two/or more different genes contribute to a single phenotype and their effects are not merely additive"/ "Polygenic trait"). There is an epistatic phenotype:
   with a starting value of 20.0, that can only be 21.0 if all four nucleotides on four
