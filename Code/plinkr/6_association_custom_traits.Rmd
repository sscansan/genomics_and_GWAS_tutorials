...
The function `create_random_trait()` will generate all three tables (.ped, .map, phenotype table).
Haaving all three tables we can then use the finction `assoc_qt()` to detect associations between trait and genotype.
...
Additive traits like height, weight... phenotypes influenced by a number of genes.
